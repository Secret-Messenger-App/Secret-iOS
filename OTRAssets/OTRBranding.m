...
static NSString *const SecretCertificate = @"SecretCertificate";
...
/** The default SSL cert (*.secret.me) */
+ (NSString*) secretCert {
    return [[self defaultPlist] objectForKey:SecretCertificate];
}
...
+ (BOOL) matchesUpstream {
    return [[[NSBundle mainBundle] bundleIdentifier] containsString:@"ms.sec.ms"];
}
