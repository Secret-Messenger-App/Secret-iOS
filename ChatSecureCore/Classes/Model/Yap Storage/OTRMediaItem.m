...
+ (instancetype) incomingItemWithFilename:(NSString*)filename
    ...
    DDLogInfo(@"incomingItemWithFilename mimeType: %@", mimeType);
    ...
    NSRange streamRange = [mimeType rangeOfString:@"application/octet-stream"];
    ...
    } else if (streamRange.location == 0 || videoRange.location == 0) {
...
+ (nullable instancetype) mediaItemForMessage:(id<OTRMessageProtocol>)message transaction:(YapDatabaseReadTransaction*)transaction {
    ...
    //DDLogError(@"mediaItemForMessage");
    return item;
}
...
- (NSString*) displayText {
    ...
    if ([self isKindOfClass:[OTRImageItem class]]) {
        ...
        item = [NSString stringWithFormat:@"%@ %@",[NSString fa_stringForFontAwesomeIcon:FAFileImageO], PICTURE_MESSAGE_STRING()];

    } else if ([self isKindOfClass:[OTRVideoItem class]]) {
        //item = [NSString stringWithFormat:@"🎥 %@", VIDEO_MESSAGE_STRING()];
        item = [NSString stringWithFormat:@"%@ %@",[NSString fa_stringForFontAwesomeIcon:FACamera], VIDEO_MESSAGE_STRING()];
    } else if ([self isKindOfClass:[OTRAudioItem class]]) {
        //item = [NSString stringWithFormat:@"🔊 %@", AUDIO_MESSAGE_STRING()];
        item = [NSString stringWithFormat:@"%@ %@",[NSString fa_stringForFontAwesomeIcon:FAMicrophone], AUDIO_MESSAGE_STRING()];
    } else {
        NSURL *url = [NSURL URLWithString:self.filename];
        if (url) {
            //item = [NSString stringWithFormat:@"🔗 %@", url.absoluteString];
            item = [NSString stringWithFormat:@"%@ %@",[NSString fa_stringForFontAwesomeIcon:FAGlobe], url.absoluteString];
