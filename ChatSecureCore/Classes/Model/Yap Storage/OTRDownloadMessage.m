...
+ (NSArray<UIAlertAction*>*) actionsForMediaMessage:(id<OTRMessageProtocol>)mediaMessage sourceView:(UIView*)sourceView viewController:(UIViewController*)viewController {
    ...
    DDLogError(@"actionsForMediaMessage messageText: %@ maybeUrl: %@", messageText, maybeURL);
        
        // sometimes the scheme is aesgcm, which can't be shared normally
        if ([maybeURL.scheme isEqualToString:@"https"] || [maybeURL.scheme isEqualToString:@"http"]) {
            url = maybeURL;
        }
    }
    ...
    if (url) {
        ...
        UIAlertAction *copyLinkAction = [UIAlertAction actionWithTitle:COPY_STRING() style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
