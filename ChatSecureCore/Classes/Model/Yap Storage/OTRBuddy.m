...
- (NSString*) nickName {
    ...
    NSString *user = [self.username otr_nickName];
    if (!user.length) {
        return _displayName;
    }
    return user;
...
- (OTRMessageTransportSecurity)preferredTransportSecurityWithTransaction:(nonnull YapDatabaseReadTransaction *)transaction {
    ...
    if (!transaction) {
        @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Missing transaction for preferredTransportSecurityWithTransaction!" userInfo:nil];
    }
    /*
...
- (NSArray<OMEMODevice*>*)omemoDevicesWithTransaction:(YapDatabaseReadTransaction*)transaction {
    NSParameterAssert(transaction);
    if (!transaction) {
        @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Missing transaction for omemoDevicesWithTransaction!" userInfo:nil];
    }
...
- (id<OTRMessageProtocol>) outgoingMessageWithText:(NSString *)text transaction:(YapDatabaseReadTransaction *)transaction {
    ...
    NSArray <OMEMODevice *>*devices = [self omemoDevicesWithTransaction:transaction];
    if ([devices count] == 0) {
        DDLogInfo(@"OTRBuddy -> outgoingMessageWithText failed with no devices");
        return nil;
    }

    //DDLogInfo(@"OTRBuddy -> outgoingMessageWithText");
