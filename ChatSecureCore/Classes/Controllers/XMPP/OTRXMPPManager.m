...
- (void) setupStream
{
    ...
    //DDLogInfo(@"Unique Identifier: %@",self.account.uniqueId);
    
    // Setup Proxy?
    if ([self.account.proxyHost length] &&
        [self.account.proxyPort length] &&
        [self.account.proxyUser length] &&
        [self.account.proxyPass length]) {
        
        self.account.isUsingProxy = YES;
        
        _xmppStream = [self newProxyStream];
        
        NSString *proxyHost = self.account.proxyHost;
        NSUInteger proxyPort = [self.account.proxyPort integerValue];

        NSString *proxyUser = self.account.proxyUser;
        NSString *proxyPass = self.account.proxyPass;
        ...
        if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
            ...
            [proxyStream setProxyUsername:proxyUser password:proxyPass];
            
            DDLogInfo(@"Init Stream with Proxy (Socks5): %@", proxyHost);
        } else {
            DDLogError(@"Init Stream with Proxy failed");
            return;
        }
        
    } else {
        
        self.account.isUsingProxy = NO;
        
        _xmppStream = [self newStream];
        DDLogError(@"Init Stream without Proxy");
    }
    ...
    // We don't want to check if OTR messages can be decrypted
    ...
    //DDLogInfo(@"xmppPushModule Activated");
    
    //_serverCheck = [[ServerCheck alloc] initWithPush:<#(PushController * _Nonnull)#> pushModule:<#(XMPPPushModule * _Nonnull)#> dispatchQueue:<#(dispatch_queue_t _Nullable)#>:nil dispatchQueue:self.workQueue];
    _serverCheck = [[ServerCheck alloc] initWithDispatchQueue:nil];
    [self.serverCheck activate:self.xmppStream];
    //[self.serverCheck.xmppCapabilities addDelegate:self delegateQueue:self.workQueue];
    //DDLogInfo(@"ServerCheck Activated");
    ...
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        ...
        NSDictionary *proxyDict = @{
                                    ...
                                    (NSString *)kCFStreamPropertySOCKSUser : proxyUser,
                                    (NSString *)kCFStreamPropertySOCKSPassword : proxyPass
                                    };
        sessionConfiguration.connectionProxyDictionary = proxyDict;
        DDLogInfo(@"Init Session with Proxy (Socks5): %@", proxyHost);
    } else {
        DDLogError(@"Init Session without Proxy");
    }
    ...
    //DDLogInfo(@"streamManagement Activated");
    ...
    [self.xmppLastActivity addDelegate:self delegateQueue:self.workQueue];
...
- (BOOL) authenticateWithStream:(XMPPStream *)stream error:(NSError**)error {
    ...
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"authenticateWithStream with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"authenticateWithStream not with ProxyXMPPStream");
    }
...
- (void) goActive {
    ...
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"goActive with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"goActive NOT with ProxyXMPPStream");
    }
...
{
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"goOffline with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"goOffline not with ProxyXMPPStream");
    }
...
{
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"updateClientState with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"updateClientState not with ProxyXMPPStream");
    }
...
{
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"connectUserInitiated with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"connectUserInitiated not with ProxyXMPPStream");
    }
...
{
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"startConnection with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"startConnection not with ProxyXMPPStream");
    }
...
- (void) sendPresenceProbeForBuddy:(OTRXMPPBuddy*)buddy
{
    ...
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"sendPresenceProbeForBuddy with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"sendPresenceProbeForBuddy not with ProxyXMPPStream");
    }
...
- (void)xmppStreamDidAuthenticate:(XMPPStream *)sender
{
    ...
    NSString *deviceToken = [NSUserDefaults.standardUserDefaults objectForKey:@"OTRDeviceToken"];
    
    NSXMLElement *command = [[NSXMLElement alloc] initWithName:@"command" xmlns:@"http://jabber.org/protocol/commands"];
    ...
    NSXMLElement *value = [[NSXMLElement alloc] initWithName:@"value" stringValue:deviceToken];
    
    [field addChild:value];
    [x addChild:field];
    [command addChild:x];
    
    XMPPIQ *iq = [[XMPPIQ alloc] initWithType:@"set" child:command];
    [iq addAttributeWithName:@"to" stringValue:self.account.domain];
    [iq addAttributeWithName:@"id" stringValue:@"execute"];
...
{
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"willSendMessage with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"willSendMessage not with ProxyXMPPStream");
    }
...
#pragma mark - XMPPPush

- (void) enablePushWithToken:(TokenContainer*)token endpoint:(NSString*)endpoint
{
    //DDLogError(@"enablePushWithToken: %@", token.pushToken.tokenString);
...
{
        DDLogError(@"pushModule: readyWithCapabilities");
    ...
    if (self.account.accountType == OTRAccountTypeXMPPTor) {
        DDLogError(@"pushModule: self.account.accountType != OTRAccountTypeXMPPTor");
        return;
    }
...
{
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"OTROutgoingMessage with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"OTROutgoingMessage not with ProxyXMPPStream");
    }
...
{
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"sendChatState with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"sendChatState not with ProxyXMPPStream");
    }
...
- (OTRXMPPBuddyTimers *) buddyTimersForBuddyObjectID:(NSString *)managedBuddyObjectID
{
...
- (void) failedToConnect:(NSError *)error
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (error) {
            ...
            DDLogInfo(@"failedToConnect showConnectionErrorNotificationWithAccount: %@",error);
...
{
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"sendDeliveryReceiptForMessage with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"sendDeliveryReceiptForMessage not with ProxyXMPPStream");
    }
...
{
    if ([self.xmppStream isKindOfClass:[ProxyXMPPStream class]]) {
        DDLogVerbose(@"xmppLastActivity didReceiveResponse with ProxyXMPPStream");
    } else {
        DDLogVerbose(@"xmppLastActivity didReceiveResponse not with ProxyXMPPStream");
    }
    
    NSInteger lastActivity = -(NSUInteger)[response lastActivitySeconds];
    
    if (lastActivity != 0 && lastActivity != NSNotFound) {
        
        //DDLogError(@"lastActivity %lu", lastActivity);
        DDLogError(@"response: %@", response);
        
        NSString *jidStr = [response attributeStringValueForName:@"from"];
        //NSString *jidStr = [item attributeStringValueForName:@"jid"];
        XMPPJID *jid = [[XMPPJID jidWithString:jidStr] bareJID];
        __block OTRXMPPBuddy *buddy = nil;
        [self.databaseConnection asyncReadWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
            ...
            NSDate *lastSeenDate = [NSDate dateWithTimeIntervalSinceNow:lastActivity];
            [OTRBuddyCache.shared setLastSeenDate:lastSeenDate forBuddy:buddy];
        }];
