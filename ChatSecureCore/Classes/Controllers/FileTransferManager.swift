    ...
    private func upload(media: OTRMediaItem,
        ...
        internalQueue.async {
            ...
            if UInt(data.count) > service.maxSize,
                ...
                let imageData = image.jpegData(dataSize: .maxBytes(service.maxSize), resize: UIImage.Quality.low, jpeg: UIImage.Quality.low, maxTries: 10)
                {
                    data = imageData
            }
    ...
    @objc public func send(image: UIImage, thread: OTRThreadOwner) {
        internalQueue.async {
            ...
            OTRMediaFileManager.shared.setData(ourImageData, for: imageItem, buddyUniqueId: thread.threadIdentifier, completion: { (bytesWritten: Int, error: Error?) in
                ...
                if let imageData = image.jpegData(dataSize: .maxBytes(service.maxSize), resize: UIImage.Quality.low, jpeg: UIImage.Quality.low, maxTries: 10) {
...
enum URLScheme: String {
    ...
    case https = "https"
    case aesgcm = "aesgcm"
    static let downloadableSchemes: [URLScheme] = [.aesgcm] // .http, .https,
}
