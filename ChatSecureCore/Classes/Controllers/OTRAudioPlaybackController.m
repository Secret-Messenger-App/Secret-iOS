...
{
    if ([OTRMediaServer sharedInstance].isStarted) {
        //DDLogError(@"media server isStarted - stopping..");
        [[OTRMediaServer sharedInstance] stop];
        //DDLogError(@"media server stopped");
    }
    
    NSError *err = nil;
    BOOL mediaServerStarted = [[OTRMediaServer sharedInstance] startOnPort:0 error:&err];
        
    if (!mediaServerStarted) {
        DDLogError(@"Error starting media server from playAudioItem: %@",err);
    }
...
+ (OTRAudioPlaybackController*) shared {
    return [self sharedInstance];
}

+ (instancetype)sharedInstance
{
    static id audioPlaybackController = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        audioPlaybackController = [[self alloc] init];
    });
