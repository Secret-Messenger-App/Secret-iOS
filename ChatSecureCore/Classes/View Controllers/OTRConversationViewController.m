...
- (void) viewDidLoad
{
    ...
    self.title = @"Secret";// CHATS_STRING();
    ...
    */
    ...
    [self.KVOController observe:[OTRProtocolManager sharedInstance] keyPaths:@[NSStringFromSelector(@selector(numberOfConnectedProtocols)),NSStringFromSelector(@selector(numberOfConnectingProtocols))] options:NSKeyValueObservingOptionNew block:^(id observer, id object, NSDictionary *change) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (strongSelf.isConnecting) {
                
                NSMutableString *nameString = [NSMutableString stringWithString:@""];
                
                [nameString appendString:CONNECTING_STRING()];

                [nameString appendString:@"..."];
                
                strongSelf.title = nameString;
            } else {
                strongSelf.title = @"Secret";//CHATS_STRING();
            }
        });
    }];
}
    ...
    [[OTRDatabaseManager sharedInstance].uiConnection readWithBlock:^(YapDatabaseReadTransaction *transaction) {
        ...
        [accounts enumerateObjectsUsingBlock:^(OTRAccount * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (![obj isKindOfClass:[OTRXMPPAccount class]]) {
                ...
                OTRXMPPAccount *account = (OTRXMPPAccount *)obj;
                OTRXMPPManager *xmpp = (OTRXMPPManager*)[[OTRProtocolManager sharedInstance] protocolForAccount:account];

                if (xmpp.loginStatus == OTRLoginStatusDisconnecting ||
                    xmpp.loginStatus == OTRLoginStatusDisconnected ||
