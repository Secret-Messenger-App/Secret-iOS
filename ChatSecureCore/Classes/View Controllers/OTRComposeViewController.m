...
- (void) viewDidLoad
{
    ...
    UITableViewCell *cellLine = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
    [self.tableViewHeader addStackedSubview:cellLine identifier:nil gravity:OTRVerticalStackViewGravityMiddle height:1 callback:^(){}];
    
    // Add the "Share Invite" button
    UITableViewCell *cellShare = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
     cellShare.textLabel.text = INVITE_LINK_STRING();
     cellShare.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    [self.tableViewHeader addStackedSubview:cellShare identifier:INVITE_LINK_STRING() gravity:OTRVerticalStackViewGravityMiddle height:80 callback:^() {
        __strong typeof(weakSelf)strongSelf = weakSelf;
        [strongSelf inviteToSecret:cellShare];
    }];
    ...
    UITableViewCell *cellCreateGroup = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
    cellCreateGroup.textLabel.text = GROUP_CHAT_STRING();
    cellCreateGroup.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    [self.tableViewHeader addStackedSubview:cellCreateGroup identifier:GROUP_CHAT_STRING() gravity:OTRVerticalStackViewGravityBottom height:80 callback:^() {
        __strong typeof(weakSelf)strongSelf = weakSelf;
        [strongSelf groupButtonPressed:cellCreateGroup];
    }];
...
- (void)setupSearchController {
    ...
    searchResultsController.tableView.estimatedRowHeight = OTRBuddyInfoCellHeight;//80; // 120
    searchResultsController.tableView.rowHeight = OTRBuddyInfoCellHeight;//UITableViewAutomaticDimension;
    searchResultsController.tableView.translatesAutoresizingMaskIntoConstraints = NO;
    ...
    //searchController.searchBar.placeholder = SEARCH_STRING;
    
    //[self.tableViewHeader addStackedSubview:self.navigationItem.searchController.searchBar identifier:nil gravity:OTRVerticalStackViewGravityTop];
...
- (void) joinGroup:(id)sender {
    ...
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = @"Secret Group";
    }];
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
