...
enum TableSections: Int {
    ...
    case details
    case delete
    static let allValues = [account, details, delete]
}
...
open class AccountDetailViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override open func viewDidLoad() {
        ...
        let cancelButton = UIBarButtonItem(barButtonSystemItem: .cancel, target: self, action: #selector(doneButtonPressed(_:)))
    ...
    private func loginStatusChanged() {
        ...
        /*
        else if xmpp.loginStatus == .disconnected {
    ...
    func showDeleteDialog(account: OTRXMPPAccount, sender: Any) {
        let alert = UIAlertController(title: DELETE_ACCOUNT_TITLE_STRING(), message: account.nickName, preferredStyle: .alert)
        let cancel = UIAlertAction(title: CANCEL_STRING(), style: .cancel)
        let delete = UIAlertAction(title: OK_STRING(), style: .destructive) { (action) in
    ...
    func loginLogoutCell(account: OTRXMPPAccount, tableView: UITableView, indexPath: IndexPath) -> SingleButtonTableViewCell {
        ...
        @unknown default:
            cell.button.setTitle(ERROR_STRING(), for: .normal)
            cell.button.setTitleColor(UIColor.red, for: .normal)
        }
        return cell
    }
